...
> 3. If you are new to programming, make an effort this week to come to office hours, which are posted [here](/spring2020/times-locations/).
...
Please download the hw04 (starter) files and save them to your repo folder. Then, open the entire hw04 folder in Atom. 

## Background & Overview
Before you dive into working with JavaScript, it’s important to first take stock of your HTML and CSS files. 

### A. HTML
Note that the HTML file is pretty simple: it’s an unordered list, where each list item contains a single div tag. Each div tag has a “background-image” style property assigned to it.
...
There is also a “preview” section, which is currently hidden via the CSS, but will eventually be used to preview any of your images in full-screen mode (as a carousel): you’ll click on an individual image, and the “.preview_box” panel will open on top of your gallery with your  image loaded inside the “.featured_image” div.
...
One last thing to call out:

`<meta name="viewport" content="width=device-width, initial-scale=1.0">`

This particular “viewport” meta tag (and the attributes therein) is needed to tell your browser to honor your actual device’s dimensions (i.e. that your web page has been optimized for mobile). Otherwise, your browser will try to scale the website to a “virtual window” or “viewport,” which is usually wider than the screen. More on that here (optional reading). 

### B. CSS
#### gallery.css 
This one’s pretty simple: uses flexbox layout, and each li w/the “card” class is allocated 25% of the screen. There’s a margin of 20px separating each card  (feel free to modify it as you like). There are also two media queries that resize the “.card” elements as the screen shrinks (test it out). There’s also a :hover pseudoclass that gives the image hover a little pizzazz.
Also note how the .image selector works to display the background image within the div. Since it’s an empty div, you have to give it a width and a height, or it will be squashed (remove the width and height to see what happens).
...
As we saw in Tutorial 4, using JavaScript to add a class to an element is a common technique for adding interactivity.
...
When you’re done, test it by clicking the top, left-hand image on the screen.
...
Also attach your “next” function to the onclick event handler of the “.featured_image” element (so that clicking anywhere on the image will advance it to the next one) — for convenience.

Hint: you will need a global variable to store information about the current item being displayed. You will then need to retrieve the next card’s background image (in the .image tag). There are many ways to do this, but you may want to google the following properties:
* parentElement
* nextElementSibling
